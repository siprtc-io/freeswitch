...
char *switch_disallow_regex(switch_core_session_t *session, switch_channel_t *channel, char *regex_str, char *source)
{
        ...
        if(!zstr(regex_str)) {
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] :  api_cmd : \n%s\n", api_cmd);
...
char *switch_allow_regex(switch_core_session_t *session, switch_channel_t *channel, char *regex_str, char *source)
{
        ...
        if(!zstr(regex_str)) {
                ...
                api_cmd = switch_mprintf("api_result=${regex(%s|%s}", source, regex_str);
                switch_core_session_execute_application(session, "set", api_cmd);
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] :  api_cmd : %s\n",api_cmd);
                switch_safe_free(api_cmd);
...
char *switch_regex_manipulation(switch_core_session_t *session, switch_channel_t *channel, char *regex_str, char *source )
{
        ...
        char *api_cmd = NULL;
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] :  regex_str : %s\n",regex_str);
                ...
                while( token != NULL) {
                        ...
                        if(!zstr(api_result)) {
                                ...
                                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] :  api_result : %s\n", api_result);
                                switch_safe_free(switch_result);
                                switch_channel_set_variable(channel, "api_result", NULL);
                        }
...
static switch_status_t load_config(void)
{
        ...
        if ((settings = switch_xml_child(cfg, "settings"))) {
                ...
                for (param = switch_xml_child(settings, "param"); param; param = param->next) {
                        ...
                        if (!strcasecmp(var, "dbname")) {                  /*! odbc DBName */
                                ...
                                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : dbname [ %s ]\n", globals.dbname);
                        } else if (!strcasecmp(var, "odbc-dsn")) {         /*! odbc dsn */
                                globals.odbc_dsn = strdup(val);
                                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : odbc-dsn [ %s ]\n", globals.odbc_dsn);
                        } else if (!strcasecmp(var, "redis-profile")) {    /*! Redix Profile */
                                globals.profile = strdup(val);
                                switch_log_printf(SWITCH_CHANNEL_LOG,SWITCH_LOG_DEBUG, "[AMAZE-DP] : redis-profile [ %s ]\n", globals.profile);
                        }
                }
        }
        ...
        if (!globals.dbname) {
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : set default DB [ %s ]\n", globals.dbname);
        }
...
static int switch_regex_callback(void *ptr, int argc, char **argv, char **col)
{
        ...
        for(index = 0; index < argc ; index++) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : %s : %s\n", col[index], argv[index]);
        }
...
//      flag = 0;
        ...
        if(!zstr(sip_equipment_type) && atoi(sip_equipment_type) > 0) {
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : Proceed To Get DIALPEER Equipment Capacity Group Data SQL :: \n%s\n",sql);
...
//      switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[AMAZE-DP] : proceed to validate DIALPEER [ %s ] in schedule time.\n", argv[0]);
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : Check DIALPEER Schedule Time SQL : \n%s\n", sql);
        switch_safe_free(sql);
...
//      switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : %s\n", line);
//      switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : ============ DIALPEER INFORMATION ============\n");
//      switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : %s\n", line);
//
//      for(index = 0; index < argc ; index++) {
//              switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : %s : %s\n", col[index], argv[index]);
//      }
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : %s\n", line);
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : ============ DIAL PEER REGEX INFORMATION ============\n");
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : %s\n", line);

        switch_execute_sql_callback(globals.mutex, sql, switch_regex_callback, &regex_master);
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : DIALPEER REGEX INFORMATION SQL : \n%s\n", sql);
        switch_safe_free(sql);
        ...
        if(!zstr(regex_master.disallow_source)) {
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : DIALPEER [ %s ] Regex Status [ %s ]\n",dp_id, regex_status);
        ...
        if(dp_cgp_cps > 0) {
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : hiredis_raw_response : %s\n",hiredis_raw_response);
        ...
        if(dp_max_cps > 0) {
                ...
                if((!zstr(hiredis_raw_response)) && (strlen(hiredis_raw_response)>0 )) {
                        ...
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "[ AMAZE-DP ] : Hiredis is unable to connect redis server [ %s ], Diap Peer  [ %s ] MAX CPS will not check.\n",hiredis_raw_response, dp_id);
                }
                switch_safe_free(idname);
                switch_safe_free(hiredis_raw_response);
        ...
        if(dp_max_calls > 0 ) {
                ...
                hiredis_raw_response = switch_mprintf("%s",switch_channel_get_variable(channel, "hiredis_raw_response"));
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "[AMAZE-DP] : hiredis_raw_response : %s\n",hiredis_raw_response);
                ...
                if(!zstr(hiredis_raw_response) && (strlen(hiredis_raw_response)>0 )) {
                        if(retval != 0) {
                                ...
                                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[ AMAZE-DP ] : release [ %s ] [ %d ]\n", idname, limit_status);
                                switch_safe_free(idname);
                        ...
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "[ AMAZE-DP ] : Hiredis is unable to connect redis server [ %s ], Diap Peer  [ %s ] MAX CALL will not check.\n",hiredis_raw_response, dp_id);
                }
                switch_safe_free(idname);
                switch_safe_free(hiredis_raw_response);
        } else {
        ...
        if(dp_cgp_mc > 0 ) {
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "[AMAZE-DP] : ********* PROCEED TO SET LIMIT *********\n");
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : idname : %s\n", idname);
...
SWITCH_STANDARD_APP(switch_dialpeer_app)
{
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : AMAZE-DP_EQUIPMENT_ID,ORIGINAL_SOURCE_NUMBER, ORIGINATL_DESTINATION_NUMBER,BILLING_SOURCE_NUMBER,BILLING_DESTINATION_NUMBER\n");
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : DIALPEER Received Request From Originate Equipment [ %s ]\n", originate_equip_id);
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : DIALPEER Recievde Source Number [ %s ] From Origination Equipment [ %s ]\n", original_source_number, originate_equip_id);
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : DIALPEER Recievde Destination Number [ %s ] From Origination Equipment [ %s ]\n", original_destination_number, originate_equip_id);
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : DIALPEER Billing Source Number [ %s ] From Origination Equipment [ %s ]\n", source_billing_number, originate_equip_id);
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : DIALPEER Billing Destination Number [ %s ] From Origination Equipment [ %s ]\n", destination_billing_number, originate_equip_id);
...
//      dp_handler.session = session;                                                      /*! Assigned Session */
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[AMAZE-DP] : Origination Equipments [ %s ] DIALPEER information SQL : \n%s\n",originate_equip_id, sql);
